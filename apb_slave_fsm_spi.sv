module apb_slave_fsm (
    ...
    logic       tx_fifo_full;
    // tx_fifo_empty, tx_fifo_rd_en, tx_fifo_rd_data are now ports (declared above)
    ...
    assign addr_ctrl   = (PADDR[7:0] == CTRL_REG);
    assign addr_status = (PADDR[7:0] == STATUS_REG);
    assign addr_tx     = (PADDR[7:0] == TX_DATA);
    assign addr_rx     = (PADDR[7:0] == RX_DATA);
    assign addr_clkdiv = (PADDR[7:0] == CLKDIV);
    ...
    always_ff @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            ...
            if (apb_write && addr_ctrl) begin
                ctrl_reg_value   <= PWDATA[7:0];
            end 
            
            if (apb_write && addr_clkdiv) begin
                clkdiv_reg_value <= PWDATA[7:0];
            end
        end
    end
    ...
    always_comb begin 
        ...
        if (current_state == ACCESS && !PWRITE) begin
            case (PADDR[7:0])
                CTRL_REG:   PRDATA = {24'h0, ctrl_reg_value};
                STATUS_REG: PRDATA = {24'h0,
                    4'b0000, 
                    rx_fifo_empty,    // bit 3    
                    rx_fifo_full,     // bit 2
                    tx_fifo_empty,    // bit 1 
                ...
                TX_DATA:    PRDATA = 32'h0;
                RX_DATA:    PRDATA = {24'h0, rx_fifo_rd_data};
                CLKDIV:     PRDATA = {24'h0, clkdiv_reg_value};
